...
/* ── Portrait area ───────────────────────────────────────────────── */
  ...
  const Cmd = {
    ...
    exit() {
      ...
      if (G.isBattle() && !G.isBattleOver()) {
        _showToast('Battle is still in progress.');
      }
      _fireKey('p', 'keydown');
  ...
  function _buildHeader() {
    ...
    for (let i = 1; i <= 5; i++) {
      row.appendChild(_mkBtn(
        `${i}<br><span class="gsub">${gIcons[i - 1]}</span>`,
        `mc3-g${i}`,
        'mc3-gbtn',
        () => Cmd.selectGroup(i)
      ));
  ...
  // Stack-mode toggle button
    ...
    row.appendChild(_mkBtn('?', 'mc3-help-btn', 'mc3-toggle-btn', () => Help.toggle()));
...
// ── HUD wrapper (card row) ──────────────────────────────────
  ...
  function _buildHelpOverlay() {
    ...
    titleEl.textContent = '⚔ Controls & Help';
    const closeBtn = D.createElement('button');
      ...
      },
{
        ...
        rows: [
          ['Single tap',       'Select a unit or issue a click to the game engine at that point.'],
          ['Hold tap',         'Move all selected units to that world position. Units spread into a loose cluster.'],
          ['Hold + drag',      'Box-select: draws a dashed gold rectangle. Any unit inside is selected when you lift your finger. Mirrors PC left-drag.'],
      ...
      },
      {
        ...
        rows: [
          ...
          ['Speed',     'Movement speed on the battlefield. Averaged for groups.'],
          ['Range',     'Maximum attack range (ranged units only). Shows the first unit in a group.'],
          ['Level',     'Experience level (1–5). Bar shows progress toward next level. Shows a range (e.g. 2–4) for mixed groups.'],
          ['Total XP',  'Accumulated experience points — summed across the whole group.'],
        ],
      },
    ];
...
// Canonical display name used as the stack key in mode 1
    ...
    cycleStackMode() {
      ...
      if (btn) btn.textContent = this._stackLabels[this._stackMode];
  ...
  const Gestures = {
    ...
    _move(e) {
     ...
     if (ids.length === 1 && this._boxActive) {
        ...
        // Only render the box if the user has dragged a noticeable amount
        if (traveled >= 14) {
          const box = D.getElementById('mc3-selbox');
          if (box) {
...
_end(e) {
      ...
      for (const t of e.changedTouches) {
        ...
        } else if (elapsed >= 220 && traveled < 14) {
          // Hold + No Drag = Move To 
          this._moveToWorld(t.clientX, t.clientY);
        } else if (elapsed < 220 && traveled < 14) {
          // Quick Tap = Standard Click/Select
...
_applyBoxSelect(x1, y1, x2, y2) {
  ...
  const Loop = {
    ...
    _tick() {
      ...
      ['mc3-form-toggle', 'mc3-cmd-toggle'].forEach(id => {
