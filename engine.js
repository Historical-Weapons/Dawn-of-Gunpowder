...
// CRITICAL CHECK: Fixed to prevent Atlantis (no cities in the Ocean!)
	...
	await setLoading(65, "Adding terrain textures");
       ...
       initializeNPCs(cities, worldMap, TILE_SIZE, COLS, ROWS, PADDING_X, PADDING_Y);
		...
		if (cities.length > 0) {
			player.x = cities[0].x;
			player.y = cities[0].y;//tried cities[5] but same location of top left
		} else {
...
const player = {
			...
			//roster: Array(25).fill("Militia").map(u => ({ type: u, exp: 1 }))
				roster: [
...
let economyTick = 0;
 ...
 function update() {
    ...
    if (inBattleMode) {
        ...
        if (keys['p']) {
            // 1. SURGICAL CONDITION: Check for living enemies and player health
            ...
            if (isPlayerDead || aliveEnemies === 0) {
                // Only allow leaving if battle is "over"
                leaveBattlefield(player);
            } else {
                console.log("Cannot retreat while enemies remain!");
            }
            
            // 2. Always reset the key to prevent rapid-fire triggering
            keys['p'] = false;
        }
...
if (typeof isSieging === 'undefined' || !isSieging) {
    ...
    ctx.fillText(foodText, player.x + (ctx.measureText(goldText).width / 2) + (statGap / 2), player.y - 38);
				...
				ctx.fillText("YOU", player.x, player.y - 26);
				ctx.font = "10px Arial";
				}
...
function syncSiegeUIVisibility() {
    ...
    if (player.isSieging && siegeGui.style.display === 'none' && !inBattleMode) {
        ...
        if (currentSiege) {
            ...
            // Call your existing function to show the GUI
            showSiegeGUI(currentSiege, currentSiege.defender); 
        } else {
