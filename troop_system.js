...
init: function() {
    ...
    this.create("General", "General", ROLES.HORSE_ARCHER, true, { desc: "A master strategist shaped by the teachings of Sun Tzu and other military classics, this commander serves as the mind of the army. Surrounded by signal flags, war drums, and elite bodyguards, his presence steadies wavering lines and keeps the intricate machinery of a 13th-century combined-arms force running smoothly. Yet beyond the battlefield, most of their days are consumed by bureaucracy and paperwork, the unseen burden of command. Not expected to be in combat, the general often wears medium armor for comfort.", weightClass: WEIGHT_CLASSES.HEAVY_CAV, isRanged: true, ammo: 24, health: 150, meleeAttack: 22, meleeDefense: 5, missileBaseDamage: 14, missileAPDamage: 8, accuracy: 62, armor: ARMOR_TIERS.FULL_LAMELLAR, speed: 2.0, range: 700, morale: 95, cost: 150 });
...
// 1. Identify roles
const isPlayerGeneral = (typeof player !== 'undefined' && unit === player);
...
// This ensures they cannot stay in a 'selected' state visually or logically.
if (unit.side !== 'player') {
    unit.selected = false;
}

// 3. Logic: Draw if unit is selected (Player only now) OR if it is a General
if ((unit.selected || isGeneral) && unit.hp > 0) {
    ...
    // 4. Styling Priority
    if (isEnemyGeneral) {
        // Enemy Boss: Permanent Dim Gold (Not "Selected" Yellow)
        ctx.strokeStyle = "rgba(255, 215, 0, 0.6)"; 
        ctx.lineWidth = 2.5;
    } else if (unit.selected) {
        // Player Selection: Bright "Active" Yellow
        ctx.strokeStyle = "rgba(255, 235, 59, 0.9)";
        ctx.lineWidth = 1.5;
    } else if (isPlayerGeneral) {
        // Player General (not selected): Permanent Bright Gold
