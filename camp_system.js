...
const AMBUSH_INTERVAL = 10.0;     // seconds between ambush rolls
const AMBUSH_CHANCE   = 0.29;     //  each interval
...
if (typeof player !== "undefined" && (player.hp || 0) <= 0) {
    ...
    // Wait 2.5 seconds so the player can process their death before packing up
    setTimeout(() => {
 ...
 if (window._CAMP_COMBAT_ACTIVE) return;  // COMBAT ADDON: handled by campCombatTick
    ...
    if (typeof player !== "undefined" && (player.hp || 0) > 0) {
        CE.ambushUnits.forEach(b => {
            ...
            if (d < 40) {
                const atk = player.meleeAttack || 15;
                const dmg = (atk * 0.09 + Math.random() * 2.5) * dt;
                b.hp -= dmg;
            }
        });
    }
...
function _drawCampPlayer(ctx) {
    ...
    if (typeof drawInfantryUnit === "function") {
        drawInfantryUnit(ctx, player.x, player.y, player.isMoving, player.anim || 0,
            col, "sword_shield", false, "player", "Commander", false, 100, 0, player, 0);
    } else {
...
// --- REPLACE WITH THIS ---
    ...
    if (!CE.ambushActive) {
        ...
        if (CE.forageTimer >= 1) {
            ...
            const base = FORAGE_RATE * (1 + (player.troops || 1) * 0.05);
