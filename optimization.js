...
;(function CAPACITOR_PERF_PATCH() {
    ...
    function _install() {
        ...
        // P6: RAF cap — CRITICAL for Pixel 2. Was incorrectly commented out
        // in v2.1. A 4000×3000 world at 30fps is the target operating point
        // that ALL other patches are tuned for.
        if (IS_NATIVE) _p6_rafCap();
    ...
    function _p14_sortThrottle() {
        ...
        window.drawAllNPCs = function (ctx, drawCar, drawShp, zoom, camL, camR, camT, camB) {
            ...
            if (
                ...
                const _realSort = globalNPCs.sort; // save the real Array.prototype.sort
                globalNPCs.sort = function () { return this; }; // no-op
                _didSuppressSort = true;

                // Call original (its internal sort is now a no-op)
                _origDraw.call(this, ctx, drawCar, drawShp, zoom, camL, camR, camT, camB);

                // Restore immediately — before any other code can touch it
